...
async function initSchema() {
  ...
  const TEXTOS_HOME = [
    ...
    { clave: 'home_subtitulo_1', contexto: 'Primera línea bajo el título del hero', es: 'Traslados de larga distancia en Gran Canaria.', en: 'Long-distance transfers in Gran Canaria.', de: 'Langstreckenfahrten auf Gran Canaria.', fr: 'Transferts longue distance à Grande Canarie.', it: 'Trasferimenti a lunga distanza a Gran Canaria.', nl: 'Langeafstandsritten op Gran Canaria.', no: 'Langtransport på Gran Canaria.', sv: 'Långdistanstransfer på Gran Canaria.', fi: 'Pitkän matkan kuljetukset Gran Canarialla.' },
    { clave: 'home_subtitulo_2', contexto: 'Segunda línea bajo el título del hero', es: 'Precio fijo por ruta.', en: 'Fixed price per route.', de: 'Fester Preis pro Fahrt.', fr: 'Prix fixe par trajet.', it: 'Prezzo fisso per tratta.', nl: 'Vaste prijs per rit.', no: 'Fast pris per rute.', sv: 'Fast pris per sträcka.', fi: 'Kiinteä hinta reittiä kohti.' },
    { clave: 'home_subtitulo_3', contexto: 'Tercera línea bajo el título del hero', es: 'Chofer confirmado antes de salir.', en: 'Driver confirmed before departure.', de: 'Fahrer wird vor Fahrtantritt bestätigt.', fr: 'Chauffeur confirmé avant le départ.', it: 'Autista confermato prima della partenza.', nl: 'Chauffeur bevestigd vóór vertrek.', no: 'Sjåfør bekreftet før avreise.', sv: 'Chauffören bekräftas före avfärd.', fi: 'Kuljettaja vahvistetaan ennen lähtöä.' },
    { clave: 'home_asistente_texto',   contexto: 'Párrafo de marketing del asistente de selección de vehículo en el hero', es: '¿Viajas con tabla de surf, bicicleta, silla de ruedas u otro equipaje especial? No todos los vehículos son iguales. Nuestro asistente te dice en segundos qué categoría se adapta mejor a tu grupo.' },
    { clave: 'home_asistente_cta',     contexto: 'Enlace que abre el asistente de selección de vehículo', es: '→ Tengo equipaje especial — Ver qué vehículo me conviene' },
    { clave: 'home_asistente_cerrar',  contexto: 'Botón de cerrar el asistente de selección de vehículo', es: 'Cerrar' },
  ...
  const TEXTOS_FLOTA = [
    ...
    { clave: 'flota_selector_intro',        contexto: 'Texto introductorio que aparece en el bloque de resultados del selector cuando hay resultados', es: 'Según tus parámetros de búsqueda, los vehículos recomendados pueden ser estos:' },
    { clave: 'flota_selector_sin_resultado',  contexto: 'Texto cuando ningún vehículo cumple todos los parámetros del selector (pasajeros, maletas y equipaje especial)', es: 'No encontramos ningún vehículo que se ajuste a tu selección. Por favor, modifica los parámetros — revisa el número de pasajeros, maletas o el equipaje especial indicado.' },
    ...
    { clave: 'flota_catalogo_btn_conoce_mas', contexto: 'Texto del botón gris "Conoce más" en cada tarjeta de categoría de la página de flota', es: 'Conoce más' },
    { clave: 'cat_ficha_categoria',         contexto: 'Etiqueta "Categoría" que aparece antes del nombre del vehículo en la ficha de categoría (ej: "Categoría Business")', es: 'Categoría' },
    { clave: 'cat_modelos_referencia',      contexto: 'Etiqueta "Modelos de referencia" en la ficha de categoría, antes de listar los modelos del vehículo', es: 'Modelos de referencia' },
    { clave: 'cat_sillas_bebe',             contexto: 'Etiqueta "Sillas bebé" en el bloque de capacidad de la ficha de categoría', es: 'Sillas bebé' },
    ...
    { clave: 'flota_catalogo_titulo',       contexto: 'Título H2 del catálogo de categorías de vehículos', es: 'Nuestras Categorías de Vehículos' },
    { clave: 'flota_selector_mal_0',        contexto: 'Opción 0 maletas del selector de maletas', es: 'Sin maletas' },
    { clave: 'flota_selector_mal_1',        contexto: 'Opción 1 maleta del selector de maletas', es: '1 maleta' },
    { clave: 'flota_selector_mal_2',        contexto: 'Opción 2 maletas del selector de maletas', es: '2 maletas' },
    { clave: 'flota_selector_mal_3',        contexto: 'Opción 3 maletas del selector de maletas', es: '3 maletas' },
    { clave: 'flota_selector_mal_4',        contexto: 'Opción 4 maletas del selector de maletas', es: '4 maletas' },
...
app.get('/admin/conductores/:id/ficha', requireAdmin, asyncHandler(async (req, res) => {
  const result = await pool.query(
    `SELECT c.id, c.nombre, c.email, c.telefono, c.documento, c.direccion, c.cp, c.municipio,
            ...
            cat.nombre AS categoria, c.categoria_id
     FROM conductores c
     LEFT JOIN categorias_vehiculos cat ON cat.id = c.categoria_id
     WHERE c.id = $1`,
    [req.params.id]
...
app.post('/admin/conductores/:id/editar', requireAdmin, asyncHandler(async (req, res) => {
  ...
  if (req.body.categoria_id !== undefined) {
    const catId = parseInt(req.body.categoria_id, 10);
    valores.push(isNaN(catId) ? null : catId);
    cambios.push(`categoria_id = $${valores.length}`);
  }
...
app.post('/admin/seo/destinos/:id/fotos', requireAdmin, asyncHandler(async (req, res) => {
  ...
  let cloudinaryUrl = null;
  try {
    const destinoRes = await pool.query('SELECT nombre FROM destinos WHERE id = $1', [req.params.id]);
    const destinoNombre = destinoRes.rows[0] ? destinoRes.rows[0].nombre : `destino-${req.params.id}`;
    ...
    console.error(`Error subiendo foto destino ${req.params.id} a Cloudinary:`, e.message);
  }
  ...
  const inserted = await pool.query(
    `INSERT INTO destinos_fotos (destino_id, imagen, mime_type, nombre_archivo, alt_texto, orden, cloudinary_url)
...
app.get('/destino-foto/:fotoId', asyncHandler(async (req, res) => {
  const result = await pool.query('SELECT imagen, mime_type, cloudinary_url FROM destinos_fotos WHERE id = $1', [req.params.fotoId]);
...
// Migra fotos de destinos a Cloudinary organizadas por subcarpeta de destino
app.post('/admin/destinos/migrar-cloudinary', requireAdmin, asyncHandler(async (req, res) => {
  let ok = 0, errores = 0;
  const fotos = await pool.query(`
    SELECT df.id, df.imagen, df.nombre_archivo, df.destino_id, d.nombre AS destino_nombre
    FROM destinos_fotos df
    JOIN destinos d ON d.id = df.destino_id
    WHERE df.cloudinary_url IS NULL
  `);
  for (const foto of fotos.rows) {
    try {
      ...
      // Subcarpeta con el nombre del destino (slugificado)
      ...
      await pool.query('UPDATE destinos_fotos SET cloudinary_url = $1 WHERE id = $2', [uploadResult.secure_url, foto.id]);
      ok++;
    } catch (e) {
      console.error(`Error migrando foto destino ${foto.id}:`, e.message);
      errores++;
    }
  }
...
app.post('/admin/seo/destinos/:id/fotos/:fotoId/principal', requireAdmin, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error('[principal] Error:', e.message);
...
app.get('/admin/seo/destinos/:id/fotos/:fotoId/imagen-base64', requireAdmin, asyncHandler(async (req, res) => {
  ...
  const row = result.rows[0];
  // PostgreSQL devuelve bytea como Buffer directamente con el driver pg
  // Si viene como string hex (\x...) hay que convertirlo
  let imgBuffer;
...
app.post('/admin/seo/destinos/:id/generar-todo', requireAdmin, asyncHandler(async (req, res) => {
  ...
  // Buscar el nombre traducido en destinos_traducciones para todos los destinos (puertos y normales)
  ...
  const prompt = d.es_puerto
    ...
    : iaPrompts.GENERADOR_DESTINO_TODO(nombreParaGenerador, d.isla, nombreIdioma, reglasSlug);
...
async function generarCategoriasConIA(items, nombreIdioma) {
  ...
  for (const item of items) {
    ...
    resultado[String(item.id)] = parcial;
  }
...
app.get('/api/palabras-paginas-publico', asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    flota_h1:                   obtenerTexto('flota_h1',                    lang),
    flota_subtitulo:            obtenerTexto('flota_subtitulo',             lang),
    flota_badge_licencias:      obtenerTexto('flota_badge_licencias',       lang),
    flota_badge_eco:            obtenerTexto('flota_badge_eco',             lang),
    flota_badge_pmr:            obtenerTexto('flota_badge_pmr',             lang),
    flota_badge_deportivo:      obtenerTexto('flota_badge_deportivo',       lang),
    flota_selector_titulo:      obtenerTexto('flota_selector_titulo',       lang),
    flota_selector_bajada:      obtenerTexto('flota_selector_bajada',       lang),
    flota_selector_descripcion: obtenerTexto('flota_selector_descripcion',  lang),
    flota_selector_label_pax:   obtenerTexto('flota_selector_label_pax',    lang),
    flota_selector_label_mal:   obtenerTexto('flota_selector_label_mal',    lang),
    flota_selector_label_esp:   obtenerTexto('flota_selector_label_esp',    lang),
    flota_selector_btn:         obtenerTexto('flota_selector_btn',          lang),
    flota_catalogo_titulo:      obtenerTexto('flota_catalogo_titulo',       lang),
    flota_catalogo_cargando:    obtenerTexto('flota_catalogo_cargando',     lang),
    flota_catalogo_btn_reservar:  obtenerTexto('flota_catalogo_btn_reservar',   lang),
    flota_catalogo_btn_conoce_mas:obtenerTexto('flota_catalogo_btn_conoce_mas', lang),
    flota_catalogo_hasta:         obtenerTexto('flota_catalogo_hasta',          lang),
    flota_catalogo_pasajero:    obtenerTexto('flota_catalogo_pasajero',     lang),
    ...
    flota_selector_sin_resultado:  obtenerTexto('flota_selector_sin_resultado',  lang),
    ...
    flota_faq1_p:               obtenerTexto('flota_faq1_p',               lang),
    flota_faq1_r:               obtenerTexto('flota_faq1_r',               lang),
    flota_faq2_p:               obtenerTexto('flota_faq2_p',               lang),
    flota_faq2_r:               obtenerTexto('flota_faq2_r',               lang),
    flota_faq3_p:               obtenerTexto('flota_faq3_p',               lang),
    flota_faq3_r:               obtenerTexto('flota_faq3_r',               lang),
    flota_faq4_p:               obtenerTexto('flota_faq4_p',               lang),
    flota_faq4_r:               obtenerTexto('flota_faq4_r',               lang),
    flota_faq5_p:               obtenerTexto('flota_faq5_p',               lang),
    flota_faq5_r:               obtenerTexto('flota_faq5_r',               lang),
    flota_selector_mal_0:       obtenerTexto('flota_selector_mal_0',        lang),
    flota_selector_mal_1:       obtenerTexto('flota_selector_mal_1',        lang),
    flota_selector_mal_2:       obtenerTexto('flota_selector_mal_2',        lang),
    flota_selector_mal_3:       obtenerTexto('flota_selector_mal_3',        lang),
    flota_selector_mal_4:       obtenerTexto('flota_selector_mal_4',        lang),
    flota_selector_mal_5:       obtenerTexto('flota_selector_mal_5',        lang),
    flota_selector_esp_ninguno: obtenerTexto('flota_selector_esp_ninguno',  lang)
  });
}));
...
app.get('/admin/textos/exportar', requireAdmin, asyncHandler(async (req, res) => {
  const textos = await pool.query(`SELECT id, clave, modulo, contexto, texto_es FROM textos_interfaz ORDER BY modulo, (regexp_replace(clave, '[^0-9]', '', 'g') || '0')::int, clave`);
  const traducciones = await pool.query(`SELECT texto_id, lang_code, texto FROM textos_interfaz_traducciones`);
...
async function asignarChoferAReserva(reservaIdParam, conductor_id, motivo) {
  ...
  try {
    ...
    if (reserva.rows.length) {
      ...
      _textoLimiteCancelEmail = fechaHoraCliente(_fechaLimiteCancelEmail, r.lang_cliente || 'es');
      ...
      const _langConf1 = r.lang_cliente || 'es';
      const _txtBotonPago1 = (await obtenerFrase('frase_boton_pagar_deposito', _langConf1, 'Pagar depósito de {importe} €')).replace('{importe}', importe);
      ...
      const _pc1 = await obtenerPlantilla('cliente_confirmacion', {
        ...
        origen: r.origen || '—',
        destino: r.destino || '—',
        fecha: fechaCliente(r.fecha, _langConf1),
        hora: r.hora ? r.hora.slice(0,5) : '—',
        categoria: r.categoria_nombre || '—',
...
app.post('/admin/reservas/:id/email-confirmacion', requireAdmin, asyncHandler(async (req, res) => {
  ...
  const fechaViaje = r.fecha ? new Date(r.fecha).toLocaleDateString('es-ES', {day:'numeric', month:'long', year:'numeric'}) : '';
  const _fechaLimiteCancelEmail = calcularFechaCancelacion(new Date(r.fecha), r.hora, horas);
  const _textoLimiteCancelEmail = fechaHoraCliente(_fechaLimiteCancelEmail, r.lang_cliente || 'es');
  ...
  const _langConf2 = r.lang_cliente || 'es';
  const _txtBotonPago2 = (await obtenerFrase('frase_boton_pagar_deposito', _langConf2, 'Pagar depósito de {importe} €')).replace('{importe}', importe);
  ...
  const _pc2 = await obtenerPlantilla('cliente_confirmacion', {
    ...
    origen: r.origen || '—',
    destino: r.destino || '—',
    fecha: fechaCliente(r.fecha, _langConf2),
    hora: r.hora ? r.hora.slice(0,5) : '—',
    categoria: r.categoria_nombre || '—',
    conductor: r.conductor_nombre || '',
    importe_deposito: importe,
...
// Fechas en el formato del idioma del cliente. En español, exactamente igual que siempre.
const LOCALE_CLIENTE = { es: 'es-ES', en: 'en-GB', de: 'de-DE', sv: 'sv-SE', no: 'nb-NO', nl: 'nl-NL', it: 'it-IT', fr: 'fr-FR', fi: 'fi-FI', ru: 'ru-RU' };
function fechaCliente(fecha, lang) {
  if (!fecha) return '';
...
app.get('/api/categoria-publica/:lang/:slug', asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    textos: {
      ...
      cat_ficha_categoria:         t('cat_ficha_categoria'),
      cat_modelos_referencia:      t('cat_modelos_referencia'),
      cat_sillas_bebe:             t('cat_sillas_bebe'),
...
// llama a /api/destino-publico.
app.get('/api/ruta-publica/:lang/:slug', asyncHandler(async (req, res) => {
  const { lang, slug } = req.params;
  ...
  const urlActual = BASE_URL + '/' + lang + '/' + (SECCIONES_TRASLADO[lang] || 'traslado') + '/' + slug;
  const { schemaTaxiService, schemaBreadcrumb } = construirSchemaRuta(
    Object.assign({}, seo, { origen: origenTraducido, destino: destinoTraducido }),
    precios.rows, nombreMarca, globales, BASE_URL, urlActual
  );

  // Textos de interfaz para la página de ruta
  const t = function (clave) { return obtenerTexto(clave, lang); };
      ...
      if (tipo === 'email') {
        await pool.query(
          `INSERT INTO plantillas_comunicacion_traducciones
             (plantilla_clave, lang_code, asunto_email, cuerpo_email, generado_por_ia, revisado_email, actualizado_en)
        ...
        await pool.query(
          `INSERT INTO plantillas_comunicacion_traducciones
             (plantilla_clave, lang_code, cuerpo_whatsapp, generado_por_ia, revisado_wa, actualizado_en)
